...
2. Fetch protein database from user-supplied fasta file (`python` script)
...
df_sample_sheet <- read_tsv(
  ...
  mutate(raw_file = paste0("..", str_sub(raw_file, -35L, -1L)))
```
...
plot_quant_pept <- df_feature_level_data %>%
  ...
  ggplot(aes(x = n_proteins, y = paste(GROUP, replicate), fill = GROUP, color = GROUP)) +
  geom_col(color = "white") +
  geom_text(aes(label = n_proteins), hjust = -0.2) +
...
plot_feat_per_prot <- df_protein_level_data %>%
  ...
  custom_theme(legend.position = "bottom") +
  scale_fill_manual(values = colorRampPalette(custom_colors[3:2])(6))
...
plot_missing_1 <- df_feature_level_data %>%
  ...
  custom_theme(legend.position = "bottom") +
  scale_fill_manual(values = custom_colors[c(1, 2)])
...
- If figure is empty, no proteins have been quantified with missing features at all
...
- If figure is empty, no proteins were flagged as problematic by `MSstats` at all
...
plot_stepplot <- df_protein_level_data %>%
  ...
  mutate(auc = sum(n_samples * n_proteins)/(sum(n_samples[-1]) * max(n_proteins))) %>%
  ggplot(aes(x = n_proteins, y = n_samples)) +
  geom_step(color = custom_colors[1], linewidth = 1, direction = "vh") +
  geom_label(
    aes(x = 0, y = 0, label = paste0("AUC = ", round(auc, 2))),
...
```{r, echo = FALSE, warning = FALSE, fig.width = figwidth, fig.height = 12}
if (nrow(df_model_qc) == 0) {
  print("No model QC data available. QQ plots cannot be generated.")
} else {
  plot_qq <- df_model_qc %>%
    filter(Protein %in% list_diff_abund_prot) %>%
...
if (nrow(df_comparison_result) == 0) {
  print("No comparison results available. Volcano plot cannot be generated.")
} else {
...
if (nrow(df_comparison_result) == 0) {
  print("No comparison results available.\nList of differentially abundant proteins cannot be generated.")
} else {
...
if (exists("df_significant") && nrow(df_significant)) {
...
- fragpipe binary: `r snakemake@config$fragpipe$executable`
