...
Protein abundance calculated by MSstats after normalization. The default is per-group median normalization.
Normalization options can be changed in the `config.yml` specified when executing the pipeline.
...
Each protein represents the middle value of one of 10 abundance ranges from low to high.
...
This entails for example conditions characterized by different variables simultaneously, such as `minimal medium` and `stationary phase` vs `rich medium` and `logarithmic phase` vs `...` etc.
...
The table is in long format, that means quantifications are shown in one column and descriptors (e.g. comparisons) in another.
